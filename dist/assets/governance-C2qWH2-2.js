/* empty css               */import"./modulepreload-polyfill-DfS4ul37.js";import{t as e}from"./provider-BwND5Fqf.js";import{a as t,c as n,l as r,n as i,r as a,t as o}from"./wallet-Bo5h3zyX.js";import{i as s,o as c,r as l,s as u}from"./xrpl-7m6Y5567.js";import{t as d}from"./nav-CdH8gFKt.js";var f=`truve_governance_records_v1`;function p(){let e=localStorage.getItem(f);if(!e)return[];try{return JSON.parse(e)}catch{return[]}}function m(e){localStorage.setItem(f,JSON.stringify(e))}function h(e){return p().filter(t=>t.userId===e)}function g(e){let t=p(),n=t.findIndex(t=>t.id===e.id);n>=0?t[n]=e:t.unshift(e),m(t)}var _=`truve_governance_round_1`,v=`usr_demo_001`,y=`proposal_q3_treasury_allocation`,b={seed:1,sprout:2,forest:3},x=document.getElementById(`top-nav`);x&&(x.innerHTML=d(`governance`));var S=document.getElementById(`governance-eligibility`),C=document.getElementById(`governance-options`),w=document.getElementById(`governance-results`),T=document.getElementById(`governance-wallet-status`),E=document.getElementById(`governance-qr-wrap`),D=document.getElementById(`governance-connect-btn`),O=document.getElementById(`governance-disconnect-btn`),k=document.getElementById(`governance-tx-log`);function A(e){let t=localStorage.getItem(_);if(!t)return e.reduce((e,t)=>(e[t]=0,e),{});try{let n=JSON.parse(t);return e.reduce((e,t)=>(e[t]=Number(n[t]??0),e),{})}catch{return e.reduce((e,t)=>(e[t]=0,e),{})}}function j(e){localStorage.setItem(_,JSON.stringify(e))}function M(e,t=!1){T&&(T.className=t?`notice error mt-12`:`notice mt-12`,T.textContent=e)}function N(e,t){E&&(E.innerHTML=`
  ...
  `)}function P(){E&&(E.innerHTML=``)}function F(){let e=i();if(!e){M(`지갑 미연결`);return}M(`연결됨: ${e.account}`)}function I(e,t){w&&(w.innerHTML=[...e].sort((e,n)=>(t[n.id]??0)-(t[e.id]??0)).map(e=>{let n=t[e.id]??0;return`
      ...
      `}).join(``)}async function R(){try{M(`Xaman SignIn 요청 생성 중...`);let e=await c();N(e.qrPngUrl,e.deepLink);let t=await u(e.uuid);if(!t.signed||!t.account){M(`지갑 연결이 취소되었습니다.`,!0);return}a({account:t.account,connectedAt:new Date().toISOString(),lastPayloadUuid:e.uuid}),r(t.account),P(),F()}catch(e){M(e instanceof Error?e.message:`지갑 연결 실패`,!0)}}function z(){o(),P(),F()}async function B(){let r=await e(),[a,o,c]=await Promise.all([r.userRepository.getProfile(v),r.donationRepository.listDonationsByUser(v),r.foundationRepository.list()]);if(!a||!S||!C)return;let d=o.filter(e=>e.nftStatus===`minted`&&e.proofNftId).length,f=d>0,p=b[a.tier]??1;S.className=f?`notice`:`notice error`,S.innerHTML=f?`투표 가능: ${a.displayName} · NFT ${d}개 보유 · 티어 <strong>${a.tier.toUpperCase()}</strong> · 가중치 <strong>${p}표</strong>`:`투표 불가: Proof NFT 보유가 확인되지 않았습니다. 기부 후 NFT를 발급받아 참여해주세요.`;let m=c.filter(e=>[`fnd_truve-community`,`fnd_green-earth`,`fnd_next-class`,`fnd_relief-now`].includes(e.id)),h=A(m.map(e=>e.id)),_=await t(y);_.length>0&&_.forEach(e=>{h[e.candidateId]=e._sum.weight??0}),I(m,h),L(),F(),D?.addEventListener(`click`,()=>{R()}),O?.addEventListener(`click`,()=>{z()}),C.innerHTML=m.map(e=>`
      ...
      `).join(``),C.querySelectorAll(`[data-vote-id]`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=e.dataset.voteId;if(!t||!f)return;let r=i();if(!r){window.alert(`먼저 Xaman 지갑을 연결해주세요.`);return}let a=m.find(e=>e.id===t);if(a)try{e.disabled=!0;let i=await s({account:r.account,destination:a.walletAddress,amountDrops:`1`,memoType:`TRUVE_GOV_VOTE`,memoData:JSON.stringify({proposalId:y,candidateId:t,weight:p,userId:v,createdAt:new Date().toISOString()}).slice(0,230)});N(i.qrPngUrl,i.deepLink);let o=await u(i.uuid);if(!o.signed||!o.txHash){window.alert(`투표 트랜잭션 서명이 취소되었습니다.`);return}let c=await l(o.txHash),d=c.validated?`validated`:`signed`;h[t]=(h[t]??0)+p,j(h),I(m,h),g({id:`gov_${Date.now()}`,userId:v,proposalId:y,candidateId:t,candidateName:a.name,weight:p,txHash:o.txHash,explorerUrl:c.explorerUrl,validationStatus:d,createdAt:new Date().toISOString()}),n({xrplAccount:r.account,proposalId:y,candidateId:t,candidateName:a.name,weight:p,txHash:o.txHash}),L(),window.alert(`투표가 온체인에 기록되었습니다. (${d})`)}catch(e){window.alert(e instanceof Error?e.message:`투표 처리 중 오류가 발생했습니다.`)}finally{e.disabled=!1}})})}B();