...
const credentialIssuerAddress = process.env.XRPL_TESTNET_CREDENTIAL_ISSUER_ADDRESS ?? process.env.XRPL_TESTNET_RLUSD_ISSUER_ADDRESS ?? "";
const credentialIssuerSeed = process.env.XRPL_TESTNET_CREDENTIAL_ISSUER_SEED ?? process.env.XRPL_TESTNET_RLUSD_ISSUER_SEED ?? "";
...
function getCredentialIssuer() {
    ...
    const wallet = xrpl_1.Wallet.fromSeed(credentialIssuerSeed);
...
function normalizeCredentialIssueInput(input) {
    ...
    if (!(0, xrpl_1.isValidClassicAddress)(subject)) {
...
async function verifyDonationPaymentForCredential(client, input) {
...
app.post("/api/credentials/issue", async (req, res) => {
    ...
    try {
        ...
        res.json({
            ...
            message: "XLS-70 CredentialCreate submitted. The donor must sign CredentialAccept in Xaman to make it valid.",
        });
    }
    catch (error) {
        res.status(500).json({
...
app.get("/api/xrpl/credential", async (req, res) => {
    ...
    const client = new xrpl_1.Client(xrplTestnetWs);
    try {
        if (!(0, xrpl_1.isValidClassicAddress)(subject) || !(0, xrpl_1.isValidClassicAddress)(issuer) || !credentialType) {
            res.status(400).json({
                exists: false,
                accepted: false,
        ...
        const node = credential.result?.node ?? credential.result?.entry ?? credential.result;
        ...
        res.json({
            ...
            credentialType,
            flags,
            ledgerIndex: credential.result?.ledger_index,
            index: node?.index,
...
app.post("/api/db/donations", async (req, res) => {
    try {
        const { xrplAccount, amountKrw, allocations, txHash, explorerUrl, receiptId, evidenceHash, complianceHash, asset, amountAsset, credential, } = req.body ?? {};
...
app.patch("/api/db/donations/:id", async (req, res) => {
    try {
        ...
        const data = Object.fromEntries(Object.entries(req.body ?? {}).filter(([key]) => allowedKeys.includes(key)));
